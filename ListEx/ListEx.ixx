...
namespace LISTEX::GDIUT {
	...
	[[nodiscard]] constexpr auto FontPointsFromPixels(long iSizePixels) -> float {
		constexpr auto flPointsInPixel = 72.F / USER_DEFAULT_SCREEN_DPI;
		return iSizePixels * flPointsInPixel;
	}
...
auto CListExHdr::OnDPIChangedAfterParent()->LRESULT
{
	...
	const auto flFontSizePoints = FontPointsFromScaledPixels(-GetFontSize());
...
{
	const auto flFontSizePoints = FontPointsFromScaledPixels(-GetFontSize()) + (fInc ? 1 : -1);
	SetFontSizeInPoints(flFontSizePoints);
}
